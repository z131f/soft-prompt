...
model = LlavaNextForConditionalGeneration.from_pretrained(model_name, cache_dir=model_cache, device_map='auto', torch_dtype=torch.float16)
...
for temp_key in inputs.keys():
    inputs[temp_key] = inputs[temp_key].to("cuda:2")
