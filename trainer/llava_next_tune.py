...
class llava_next_tune_trainer():
    def __init__(self, config, logger):
        ...
        # print('load model ...')
        self.model = ModifiedLlavaNext.from_pretrained(model_site, config=model_config, cache_dir=model_cache, device_map=config['device_map'], 
                                                       patch_num=patch_num, torch_dtype=torch.bfloat16, extra_fun=config['extra_fun'])
        # print('load model processor ...')
        self.processor = LlavaNextProcessor.from_pretrained(model_site, cache_dir=model_cache+'/llava_next_processor', use_fast=True, num_additional_image_tokens=1 + 1)
    ...
    def eval(self, is_print=False, ans_add=''):
        ...
        for idx, batch in tqdm(enumerate(eval_dataloader), desc="正在评估", total=len(self.eval_dataset)):
            ...
            with torch.no_grad():
                ...
                parts = full_generated_text.split("[/INST]", 1) # 1 表示只分割一次
